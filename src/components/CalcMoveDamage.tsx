...
function CalcMoveDamage ({gens, atkPkm, defPkm}) {
  ...
  const calcDamageRange = (result) => {
    ...
    if ( drain ) {
      returnObj[move] = `${returnObj[move]} (${parseFloat((atkackLowRange*drain[0]/drain[1]).toFixed(1))}% - ${parseFloat((atkackHighRange*drain[0]/drain[1]).toFixed(1))}% recovered)`
    }
    if ( recoil ) {
      returnObj[move] = `${returnObj[move]} (${parseFloat((atkackLowRange*recoil[0]/recoil[1]).toFixed(1))}% - ${parseFloat((atkackHighRange*recoil[0]/recoil[1]).toFixed(1))}% recoiled)`
    }
    return returnObj;
  };
